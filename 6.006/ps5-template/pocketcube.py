...
def solve(config):
    ...
    while len(frontier) != 0 and len(frontier2) != 0 and len(set(parent.keys()).intersection(set(parent2.keys()))) == 0:
        frontier = explore_frontier(frontier, parent, True)
        frontier2 = explore_frontier(frontier2, parent2, True)
    print('Searched %s reachable configurations' % len(parent))

    if len(set(parent.keys()).intersection(set(parent2.keys()))) == 0:
        return None
    # Check whether solved state visited and reconstruct path
    mid = list(set(parent.keys()).intersection(set(parent2.keys())))[0]
    path1 = path_to_config(mid, parent)
    path2 = path_to_config(mid, parent2)
    path2.reverse()

    return moves_from_path(path1) + moves_from_path(path2)
