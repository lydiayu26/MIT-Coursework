...
class BlackJackHand:
    ...
    @staticmethod
    def best_val(cards, maxPoints = 21):
        """
        ...
        #loops through cards
        for card in cards:
            #adds value of card to total if it's not an ace
            if card.get_rank() != 'A':
                nonAces.append(card)
                total += card.get_val()
            #if the card is an ace, add 11 to total and add 1 to numAces
            else:
                numAces += 1
                total += 11
        
        acesLeft = numAces        
        #if you have gone over 21 by adding 11 for all the aces
        if total > 21:
           #subtract 10 for each ace you have left until you get a total under 21
            while total > 21 and acesLeft > 0:
                total -= 10
                acesLeft -= 1
    ...
    def mimic_dealer_strategy(self):
        ...
        """
        if BlackJackHand.best_val(self.get_player_cards()) < 17:
            return "hit"
        return "stand"
    ...
    def peek_strategy(self):
        ...
        str, "hit" or "stand" representing the player's decision
        """
        if BlackJackHand.best_val(self.get_player_cards()) < BlackJackHand.best_val(self.get_dealer_cards()):
            return "hit"
        return "stand"
    ...
    def simple_strategy(self):
        ...
        str, "hit" or "stand" representing the player's decision 
        """
        if BlackJackHand.best_val(self.get_player_cards()) >= 17 or \
            ((12 <= BlackJackHand.best_val(self.get_player_cards()) <= 16) and (2 <= self.get_dealer_upcard().get_val() <= 6)):
            return "stand"
        return "hit"
    ...
    def play_player(self, strategy):
        ...
        while strategy(self) != "stand":
            #keep adding cards to the player until they stand or bust
            self.player.append(self.deck.deal_card())  
            
            if BlackJackHand.best_val(self.get_player_cards()) > 21:
                raise Busted
    ...
    def play_dealer(self):
        ...
        """
        while BlackJackHand.best_val(self.get_dealer_cards()) < 17: #and BlackJackHand.best_val(self.get_dealer_cards()) <= 21:
            #keep adding cards to dealer until their total is > 17 or they bust
            self.dealer.append(self.deck.deal_card())
            if BlackJackHand.best_val(self.get_dealer_cards()) > 21:
                raise Busted
...
def play_hand(deck, strategy, bet=1.0):
    ...
    #if dealer has blackjack w first 2 cards, return 0
    if hand.best_val(hand.get_dealer_cards()) == 21 and hand.best_val(hand.get_player_cards()) != 21:
        return 0     
    #if player has blackjack w first 2 cards, return bet*2.5
    elif hand.best_val(hand.get_player_cards()) == 21 and hand.best_val(hand.get_dealer_cards()) != 21:
        return bet * 2.5
    #if both have blackjack w first 2 cards, return original bet
    elif hand.best_val(hand.get_dealer_cards()) == 21 and hand.best_val(hand.get_player_cards()) == 21:
        return bet
    
    #play player; if they bust, return 0    
    try:
        hand.play_player(strategy)
    except:
        return 0

    #assuming that player doesn't bust, play dealer; if they bust, return 2*bet
    try:
        hand.play_dealer()
    except:
        return 2 * bet
    
    playerVal = hand.best_val(hand.get_player_cards())
    dealerVal = hand.best_val(hand.get_dealer_cards())
    
    #if player has more than dealer, return 2*bet
    if playerVal > dealerVal:
        return 2 * bet
    #if they tie, return original bet
    elif playerVal == dealerVal:
        return bet
    #if dealer has more than player, return 0
    else:
        return 0
...
def run_simulation(strategy, bet=2.0, num_decks=8, num_hands=20, num_trials=100, show_plot=False):
    ...
    for trial in range(num_trials):
        ...
        #adds returned money to totalEarned and amt bet to totalBet each time a hand is played
        for hand in range(num_hands):
            totalEarned += play_hand(deck, strategy, bet)
            totalBet += bet
        rate = 100 * (totalEarned - totalBet)/totalBet
        rateReturns.append(rate)
        totalRates += rate
        
    avgRate = totalRates/num_trials
    stdev = np.std(rateReturns)
    return (rateReturns, avgRate, stdev)
...
def run_all_simulations(strategies):
    ...
    for strategy in strategies:
        ...
        all_sims.append(sim[0])
    ax.boxplot(all_sims, showfliers=True, showmeans = True, labels=['mimic', 'peek', 'simple'])
    ax.set_title('Player ROI Using Different Strategies')
