...
training = [cobra, rattlesnake, boa, chicken, dartFrog]
testing = [zebra, alligator]
...
#compareAnimals(training + testing, 3)
...
def buildTitanicExamples(fileName, toScale = False):
    data = getTitanicData(fileName, toScale)
    examples = []
    for i in range(len(data['class'])):
        p = Passenger(data['class'][i], data['age'][i],
                      ...
                      data['name'][i])
        examples.append(p)
    print('Finished processing', len(examples),
          'passengers\nwith toScale =', toScale, '\n')    
    return examples
...
examples = buildTitanicExamples('TitanicPassengers.txt', False)
