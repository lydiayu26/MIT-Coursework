...
class ps3_P1A(unittest.TestCase):
    """test the RectangularRoom base class"""    
    ...
    def test_get_num_tiles(self): #TODO - DONE 
        ...
        for i in range(11):
            ...
            room_num_tiles = ps3.RectangularRoom(width, height, dirt_amount).get_num_tiles()
...
class ps3_P3(unittest.TestCase):
    """This  tests RectangularRoom and Simple robot in various ways"""
    ...
    def test_BoundaryConditions(self):
        "Test strict inequalities in random positions for the RectangularRoom and SimpleRobot"
        for m in range(7000):
    ...
    def test_update_position_and_cleanSimpleRobot(self): #TODO - done
        "Test SimpleRobot.update_position_and_clean"
...
"""The SIMULATION_TIME_LIMIT, SimulationThread class and SimulationTester class
    ...
    and passes the tests for SimpleRobot and RectangularRoom, there should not be any
    additional issue.
"""
...
class ps3_P5_Simple(SimulationTester):
    """test the simulation time cleaning the RectangularRoom with a SimpleRobot"""
...
class ps3_P5_PlusCat(SimulationTester):
    """test the simulation time cleaning the RectangularRoom with a RobotPlusCat"""
    def testSimulation1(self):
        "Test cleaning 100% of a 5x5 room with RobotPlusCat"
        x = ps3.run_simulation(1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.RobotPlusCat)
        self.assertTrue(165 <= x <= 210, "Simulation output was outside of 99.7% confidence interval!\n")    
    def testSimulation2(self):
        "Test cleaning 75% of a 10x10 room with RobotPlusCat"
        x = ps3.run_simulation(1, 1.0, 1, 10, 10, 1, 0.75, 100, ps3.RobotPlusCat)
        self.assertTrue(220 <= x <=247, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n") #todo - look into this 
    def testSimulation3(self):
        "Test cleaning 90% of a 10x10 room with RobotPlusCat"
        x = ps3.run_simulation(1, 1.0, 1, 10, 10, 1, 0.9, 100, ps3.RobotPlusCat)
        self.assertTrue(358 <= x <=397, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation4(self):
        "Test cleaning 100% of a 5x5 room with 2 RobotPlusCats"
        x = ps3.run_simulation(2, 1.0, 2, 5, 5, 5, 1.0, 100, ps3.RobotPlusCat)
        self.assertTrue(172 <= x <=200, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation5(self):
        "Test cleaning 75% of a 10x10 room with 4 RobotPlusCats"
        x = ps3.run_simulation(4, 1.0, 3, 10, 10, 5, 0.75, 100, ps3.RobotPlusCat)
        self.assertTrue(101 <= x <=111, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation6(self):
        "Test cleaning 90% of a 10x10 room with 5 RobotPlusCats"
        x = ps3.run_simulation(5, 1.0, 3, 10, 10, 10, 0.9, 100, ps3.RobotPlusCat)
        self.assertTrue(204 <= x <=218, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")

class ps3_P5_Boosted(SimulationTester):
    """test the simulation time cleaning the RectangularRoom with a RobotPlusCat"""
    def testSimulation1(self):
        "Test cleaning 100% of a 5x5 room with BoostedRobot"
        x = ps3.run_simulation(1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.BoostedRobot)
...
if __name__ == "__main__":
    ...
    suite.addTest(unittest.makeSuite(ps3_P5_PlusCat))
    suite.addTest(unittest.makeSuite(ps3_P5_Boosted))
