...
class TestPS4B(unittest.TestCase):
    ...
    def test_message_make_shift_dicts(self):
        sd_list = self.msg1.make_shift_dicts([0, 4])
    ...
    def test_message_apply_shifts(self):
        error_msg = "Encoding message '%s' with shift of %s failed. Expected %s, got %s"

        sd1 = self.msg1.make_shift_dicts([3,2])
        sd2 = self.msg2.make_shift_dicts([1,3])
        sd3 = self.msg3.make_shift_dicts([1,0])

        msg1_encoded = self.msg1.apply_shifts(sd1)
        msg2_encoded = self.msg2.apply_shifts(sd2)
        msg3_encoded = self.msg3.apply_shifts(sd3)
    ...
    def test_plaintext_message_attributes(self):
        ...
        new_ed1 = new_eds[0]
        self.assertEqual(52, len(new_ed1), "get_encryption_dict() should return a copy of the encryption_dict attribute, but your code returns the original.")
    ...
    def test_plaintext_message_modify_shifts(self):
        self.ptmsg1.modify_shifts([0, 0])

        actual_shift = self.ptmsg1.get_shifts()
        self.assertEqual([0,0], actual_shift, "After calling modify_shifts(), the value of self.shift was not updated correctly: expected %s, got %s" % (0, actual_shift))
        ...
        self.assertEqual(expected_ed, actual_ed, 'After calling modify_shifts(), the encryption_dict attribute was not updated correctly.')

        actual_message = self.ptmsg1.get_encrypted_message_text()
        expected_message = self.text1
        self.assertEqual(expected_message, actual_message, 'After calling modify_shifts(), the encrypted_message_text attribute was not updated correctly. Expected %s, got %s' % (expected_message, actual_message))
...
failure_messages = {
    ...
    'test_message_make_shift_dicts' : 'Message.make_shift_dicts() does not correctly build a shift dictionary.',
    'test_message_apply_shifts' : 'Message.apply_shifts() does not correctly encrypt messages.',
    ...
    'test_plaintext_message_modify_shifts' : 'PlaintextMessage.modify_shifts() does not properly update object attributes',
...
point_values = {
    ...
    'test_message_make_shift_dicts' : 0.5,
    'test_message_apply_shifts' : 0.5,
    'test_plaintext_message_attributes' : 0.25,
    'test_plaintext_message_modify_shifts' : 0.5,
